...
* **Paralela:** Componente compressional (na direção do campo médio).
* **Radial/Poloidal:** Componente perpendicular apontando para fora (em direção ao gradiente de L-shell).
* **Azimutal/Toroidal:** Componente perpendicular apontando para leste (direção azimutal).

---

## 2. Passo a Passo do Processamento

### Passo 1: Limpeza do Campo Elétrico
O movimento do satélite corta as linhas de campo magnético a alta velocidade, gerando um campo elétrico artificial induzido que mascara as ondas reais. A primeira coisa a fazer é remover esse efeito do campo elétrico medido:
...
* **Campo de Fundo:** O que sobra abaixo da frequência de corte (tendência orbital).
* **Perturbações:** O sinal oscilatório puro acima da frequência de corte.

### Passo 3: Construção do Triedro MFA (Geometria)
A matriz de rotação ponto a ponto é calculada usando exclusivamente o **campo magnético de fundo** e a **posição do satélite** em coordenadas GSM.

Os vetores unitários de cada componente são definidos pelas seguintes relações matemáticas:

1. **Vetor unitário paralelo ($\hat{e}_\parallel$):** Alinhado ao campo de fundo médio.

$$ \hat{e}_\parallel = \frac{\vec{B}_0}{|\vec{B}_0|} $$

2. **Vetor unitário azimutal ($\hat{e}_\phi$):** Perpendicular au plano formado pelo campo magnético e a posição do satélite.
...
Finalmente, montamos a matriz de rotação $R$ composta pelos vetores unitários calculados:

$$ R = \begin{bmatrix} \hat{e}_\parallel \\ \hat{e}_r \\ \hat{e}_\phi \end{bmatrix} $$

Projetamos as perturbações isoladas no passo 2 para obter os componentes físicos finais no sistema alinhado:

$$ \vec{V}_{MFA} = R \cdot \vec{V}_{GSM} $$

Isso é aplicado de forma idêntica tanto para as flutuações magnéticas quanto elétricas.
